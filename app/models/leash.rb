class Leash < ApplicationRecord
  ...
  validates :friendship, uniqueness: { message: 'was already used in a leash. Are you already a pet/owner?' }

  scope :held_by, ->(user) { where(master: user) }
  scope :leashing, ->(user) { where(pet: user) }

  def only_one_master_per_pet
    errors.add(:pet, "already has owner #{master.username}.") if Leash.leashing(pet).first&.valid?
  end
